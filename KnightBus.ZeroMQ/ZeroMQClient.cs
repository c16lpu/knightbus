...
{
    public interface IZeroMQQueueClient
    {
        Task<int> GetQueueCountAsync();
        Task DeadLetterAsync(ZeroMQQueueMessage message);
        Task<int> GetDeadLetterCountAsync();
        Task<List<ZeroMQQueueMessage>> PeekDeadLettersAsync<T>(int count) where T : IZeroMQCommand;
        Task<ZeroMQQueueMessage> ReceiveDeadLetterAsync<T>() where T : IZeroMQCommand;
        Task RequeueDeadLettersAsync<T>(int count, Func<T, bool> shouldRequeue) where T : IZeroMQCommand;
        Task CompleteAsync(ZeroMQQueueMessage message);
        Task AbandonByErrorAsync(ZeroMQQueueMessage message, TimeSpan? visibilityTimeout);
        Task SendAsync<T>(T message, TimeSpan? delay) where T : IZeroMQCommand;
        Task<List<ZeroMQQueueMessage>> GetMessagesAsync<T>(int count, TimeSpan? lockDuration) where T : IZeroMQCommand;
        Task CreateIfNotExistsAsync();
        Task DeleteIfExistsAsync();
        Task SetVisibilityTimeout(ZeroMQQueueMessage message, TimeSpan timeToExtend, CancellationToken cancellationToken);
    }

    public class ZeroMQQueueClient : IZeroMQQueueClient
    {
        ...
        public ZeroMQQueueClient(IZeroMQConfiguration configuration, IMessageAttachmentProvider attachmentProvider, string queueName)
        {
        ...
        public async Task DeadLetterAsync(ZeroMQQueueMessage message)
        {
        ...
        public async Task CompleteAsync(ZeroMQQueueMessage message)
        {
        ...
        public async Task AbandonByErrorAsync(ZeroMQQueueMessage message, TimeSpan? visibilityTimeout)
        {
        ...
        public async Task SendAsync<T>(T message, TimeSpan? delay) where T : IZeroMQCommand
        {

            var ZeroMQMessage = new ZeroMQQueueMessage(message)
            {
            ...
            if (typeof(ICommandWithAttachment).IsAssignableFrom(typeof(T)))
            {
                ...
                if (attachmentMessage.Attachment != null)
                {
                    ...
                    ZeroMQMessage.Properties[AttachmentUtility.AttachmentKey] = string.Join(",", attachmentIds);
                }
            }
        ...
        public Task<List<ZeroMQQueueMessage>> PeekDeadLettersAsync<T>(int count) where T : IZeroMQCommand
        {
            return GetMessagesAsync<T>(count, null, _dlQueue);
        }

        public async Task RequeueDeadLettersAsync<T>(int count, Func<T, bool> shouldRequeue) where T : IZeroMQCommand
        {
        ...
        public async Task<ZeroMQQueueMessage> ReceiveDeadLetterAsync<T>() where T : IZeroMQCommand
        {
        ...
        public async Task SetVisibilityTimeout(ZeroMQQueueMessage message, TimeSpan timeToExtend, CancellationToken cancellationToken)
        {
            /*var cloudQueueMessage = new CloudQueueMessage(message.QueueMessageId, message.PopReceipt);
        ...
        public Task<List<ZeroMQQueueMessage>> GetMessagesAsync<T>(int count, TimeSpan? lockDuration) where T : IZeroMQCommand
        {
            return GetMessagesAsync<T>(count, lockDuration, _queue);
        }
        /*private async Task<List<ZeroMQQueueMessage>> GetMessagesAsync<T>(int count, TimeSpan? lockDuration, CloudQueue queue) where T : IZeroMQCommand
        {
